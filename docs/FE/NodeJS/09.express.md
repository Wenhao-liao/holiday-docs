...
module.exports = router;Copy to clipboardErrorCopied
```
...
//server.js
app.set("view engine", "ejs");
app.set("views", "views")Copy to clipboardErrorCopied
//user-router
...
req.session.userID = isHasUser._id;Copy to clipboardErrorCopied
//权限文件中
...
res.clearCookie("userID");
res.status(401).send("你没有权限访问")Copy to clipboardErrorCopied
```
...
//启动服务的时候调用
open("http://127.0.0.1:3000")Copy to clipboardErrorCopied
```
